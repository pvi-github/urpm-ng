...
Vóór er iets gedownload wordt, wordt het plan afgemeten tegen elk bestandssysteem dat er een deel van moet dragen. Aan dat met `/usr` wordt toegerekend wat de binnenkomende pakketten innemen min wat de vertrekkende teruggeven, plus de pakketten die nog in gebruik zijn en waarvan de ruimte pas met de laatste reeksen vrijkomt. Aan dat met de downloadcache worden de nog op te halen RPM's toegerekend. Staat `/var` op een eigen partitie, dan krijgen beide een eigen oordeel — ze optellen zou een opwaardering weigeren waarvan de pakketten ruim in `/var` passen, alleen omdat `/usr` krap zit. Ook op één bestandssysteem worden pakketten en beslag niet opgeteld: de RPM's van elke reeks worden gewist zodra die geïnstalleerd is, dus de cache loopt leeg in het tempo waarin de nieuwe bestanden binnenkomen en de piek is de grootste van de twee, niet hun som. Daarbovenop komen één reeks tussen zijn schrijfacties en zijn commit, en een aangegeven reserve van 10% van het geïnstalleerde beslag voor wat geen enkel pakket beschrijft — wat scriptlets voortbrengen, de groei van de rpmdb. Die reserve staat op een eigen regel in plaats van in het totaal te verdwijnen. De blokken die een bestandssysteem voor root reserveert, tellen niet als beschikbaar. Past een van beide niet, dan wordt de repository-omschakeling teruggedraaid en worden de cijfers getoond met de commando's die ruimte vrijmaken waar die werkelijk ontbreekt. Wat scriptlets aanmaken — initramfs, lettertype- en pictogramcaches — hoort bij geen enkel pakket en wordt niet meegeteld.
