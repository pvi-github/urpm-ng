...
Vóór er iets gedownload wordt, wordt het plan afgemeten tegen elk bestandssysteem dat er een deel van moet dragen, en de cijfers worden naast de bevestigingsprompt getoond. Er wordt niets geweigerd: de schatting is op één gemeten opwaardering geijkt, en een onterechte weigering zou een opwaardering afbreken die alle kans maakte — na de repository-omschakeling, want daarvóór kan het plan niet bestaan.

Aan het bestandssysteem met `/usr` wordt toegerekend wat de binnenkomende pakketten innemen min wat de vertrekkende teruggeven, plus één reeks tussen zijn schrijfacties en zijn commit, plus de pakketten die nog in gebruik zijn en waarvan de ruimte pas met de laatste reeksen vrijkomt. Aan dat met de downloadcache worden de nog op te halen RPM's toegerekend. Staat `/var` op een eigen partitie, dan krijgen beide een eigen oordeel; op één bestandssysteem worden ze evenmin opgeteld, want de RPM's van elke reeks worden gewist zodra die geïnstalleerd is: de cache loopt leeg in het tempo waarin de nieuwe bestanden binnenkomen en de piek is de grootste van de twee.

Naast die schatting — nooit erin — staat een aangeraden marge van 10% van het geïnstalleerde beslag, voor wat geen enkel pakket aangeeft: de opnieuw aangemaakte initramfs, de lettertype- en pictogramcaches. Die in de behoefte verrekenen maakte van een meting een oordeel. De blokken die een bestandssysteem voor root reserveert, tellen niet als beschikbaar. Is de marge krap of het bestandssysteem ronduit te klein, dan krijgt u de cijfers en de commando's die ruimte vrijmaken waar die werkelijk ontbreekt, en beslist u zelf.
