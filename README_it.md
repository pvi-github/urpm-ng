...
Prima di ogni download, il piano viene misurato rispetto a ogni filesystem che deve ospitarne una parte. A quello che ospita `/usr` viene imputato quanto occupano i pacchetti in arrivo meno quanto restituiscono quelli che se ne vanno, più i pacchetti ancora in uso il cui spazio torna solo con gli ultimi lotti. A quello che ospita la cache dei download vengono imputati gli RPM ancora da recuperare. Quando `/var` è una partizione separata, ciascuno ha il proprio verdetto — sommarli rifiuterebbe un aggiornamento i cui pacchetti stanno comodamente in `/var` solo perché `/usr` è stretto. Su un solo filesystem payload e impronta non si sommano comunque: gli RPM di ogni lotto vengono cancellati appena è installato, quindi la cache si svuota al ritmo in cui arrivano i file nuovi e il picco è il maggiore dei due, non la loro somma. Vi si aggiungono un lotto preso fra le sue scritture e il suo commit, e una riserva dichiarata del 10% dell'impronta installata per ciò che nessun pacchetto descrive — l'output degli scriptlet, la crescita del rpmdb. Quella riserva è stampata su una riga propria invece di sparire nel totale. I blocchi che un filesystem riserva a root non contano come disponibili. Se uno dei due non ci sta, il cambio dei repository viene annullato e le cifre vengono stampate insieme ai comandi che liberano spazio dove manca davvero. Ciò che generano gli scriptlet — initramfs, cache di font e icone — non appartiene ad alcun pacchetto e non viene conteggiato.
