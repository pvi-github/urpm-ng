...
Vor jedem Download wird der Plan gegen jedes Dateisystem gemessen, das einen Teil davon aufnehmen muss. Dem Dateisystem mit `/usr` wird angerechnet, was die eingehenden Pakete belegen, minus dem, was die abgehenden zurückgeben, plus die noch laufenden Pakete, deren Platz erst mit den letzten Stapeln zurückkommt. Dem Dateisystem mit dem Download-Cache werden die noch zu ladenden RPMs angerechnet. Liegt `/var` auf einer eigenen Partition, bekommen beide ein eigenes Urteil — sie zu addieren würde eine Systemaktualisierung ablehnen, deren Pakete bequem in `/var` passen, nur weil `/usr` knapp ist. Auch auf einem einzigen Dateisystem werden Pakete und Fußabdruck nicht addiert: die RPMs jedes Stapels werden gelöscht, sobald er installiert ist, der Cache leert sich also im Takt der ankommenden Dateien und die Spitze ist der größere der beiden Werte, nicht ihre Summe. Dazu kommen ein Stapel zwischen seinen Schreibvorgängen und seinem Commit sowie eine erklärte Reserve von 10% des installierten Fußabdrucks für das, was kein Paket beschreibt — Scriptlet-Ausgaben, rpmdb-Wachstum. Diese Reserve steht in einer eigenen Zeile, statt im Gesamtwert zu verschwinden. Die Blöcke, die ein Dateisystem für root reserviert, zählen nicht als verfügbar. Passt eines von beiden nicht, wird der Repository-Wechsel zurückgenommen und die Zahlen werden mit den Befehlen ausgegeben, die dort Platz schaffen, wo er wirklich fehlt. Was Scriptlets erzeugen — initramfs, Schrift- und Symbol-Caches — gehört zu keinem Paket und wird nicht mitgezählt.
