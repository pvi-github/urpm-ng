...
Vor jedem Download wird der Plan gegen jedes Dateisystem gemessen, das einen Teil davon aufnehmen muss, und die Zahlen werden direkt vor dem Bestätigungs-Prompt ausgegeben. Nichts wird abgelehnt: die Schätzung ist an einer einzigen gemessenen Migration kalibriert, und eine falsche Ablehnung würde eine Aktualisierung abbrechen, die jede Chance hatte — nach dem Repository-Wechsel, denn vorher kann der Plan nicht existieren.

Dem Dateisystem mit `/usr` wird angerechnet, was die eingehenden Pakete belegen, minus dem, was die abgehenden zurückgeben, plus ein Stapel zwischen seinen Schreibvorgängen und seinem Commit, plus die noch laufenden Pakete, deren Platz erst mit den letzten Stapeln zurückkommt. Dem Dateisystem mit dem Download-Cache werden die noch zu ladenden RPMs angerechnet. Liegt `/var` auf einer eigenen Partition, bekommen beide ein eigenes Urteil; auf einem einzigen Dateisystem werden sie ebenfalls nicht addiert, denn die RPMs jedes Stapels werden nach seiner Installation gelöscht: der Cache leert sich im Takt der ankommenden Dateien und die Spitze ist der größere der beiden Werte.

Neben dieser Schätzung — nie darin — steht eine empfohlene Reserve von 10% des installierten Fußabdrucks, für das, was kein Paket deklariert: die neu erzeugte initramfs, die Schrift- und Symbol-Caches. Sie in den Bedarf einzurechnen machte aus einer Messung ein Urteil. Die Blöcke, die ein Dateisystem für root reserviert, zählen nicht als verfügbar. Ist die Reserve knapp oder das Dateisystem schlicht zu klein, erhält man die Zahlen und die Befehle, die dort Platz schaffen, wo er wirklich fehlt, und entscheidet selbst.
