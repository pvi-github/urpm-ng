...
Avant tout téléchargement, le plan est mesuré face à chaque système de fichiers qui doit en accueillir une part, et les chiffres sont affichés juste avant le prompt de confirmation. Rien n'est refusé : l'estimation n'est calibrée que sur une seule migration mesurée, et un refus à tort annulerait une mise à niveau qui avait toutes ses chances — après la bascule des dépôts, puisque le plan ne peut pas exister avant.

Le système de fichiers qui porte `/usr` se voit imputer ce qu'occupent les paquets entrants moins ce que rendent ceux qui partent, plus un lot pris entre ses écritures et son commit, plus les paquets en cours d'utilisation dont la place n'est rendue que par les derniers lots. Celui qui porte le cache de téléchargement se voit imputer les RPM restant à récupérer. Quand `/var` est une partition distincte, les deux ont chacun leur verdict ; sur un système de fichiers unique ils ne s'additionnent pas davantage, car les RPM de chaque lot sont supprimés dès qu'il est installé : le cache se vide au rythme où les nouveaux fichiers arrivent et le pic vaut le plus grand des deux.

À côté de cette estimation — jamais dedans — figure une marge conseillée de 10 % de l'empreinte installée, pour ce qu'aucun paquet ne déclare : l'initramfs régénéré, les caches de polices et d'icônes. L'inclure dans le besoin transformait une mesure en verdict. Les blocs qu'un système de fichiers réserve à root ne sont pas comptés comme disponibles. Si la marge est mince ou le système de fichiers franchement trop petit, vous obtenez les chiffres et les commandes qui libèrent de la place là où elle manque vraiment, et vous décidez.
