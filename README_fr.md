...
Avant tout téléchargement, le plan est mesuré face à chaque système de fichiers qui doit en accueillir une part. Celui qui porte `/usr` se voit imputer ce qu'occupent les paquets entrants moins ce que rendent ceux qui partent, plus les paquets en cours d'utilisation dont la place n'est rendue que par les derniers lots. Celui qui porte le cache de téléchargement se voit imputer les RPM restant à récupérer. Quand `/var` est une partition distincte, les deux ont chacun leur verdict — les additionner refuserait une mise à niveau dont le payload tient largement dans `/var` au motif que `/usr` est juste. Sur un système de fichiers unique, payload et empreinte ne s'additionnent pas davantage : les RPM de chaque lot sont supprimés dès qu'il est installé, donc le cache se vide au rythme où les nouveaux fichiers arrivent et le pic vaut le plus grand des deux, pas leur somme. S'y ajoutent un lot pris entre ses écritures et son commit, et une provision déclarée de 10 % de l'empreinte installée pour ce qu'aucun paquet ne décrit — ce que produisent les scriptlets, la croissance du rpmdb. Cette provision est affichée sur sa propre ligne plutôt que fondue dans le total. Les blocs qu'un système de fichiers réserve à root ne sont pas comptés comme disponibles. Si l'un des deux ne tient pas, la bascule des dépôts est annulée et les chiffres sont affichés avec les commandes qui libèrent de la place là où elle manque vraiment. Ce que génèrent les scriptlets — initramfs, caches de polices et d'icônes — n'appartient à aucun paquet et n'est pas compté.
