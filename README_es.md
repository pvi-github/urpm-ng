...
Antes de cualquier descarga, el plan se mide frente a cada sistema de archivos que debe alojar una parte, y las cifras se muestran junto al aviso de confirmación. No se rechaza nada: la estimación está calibrada sobre una única migración medida, y un rechazo equivocado abortaría una actualización que tenía todas las de ganar — después de haber cambiado los repositorios, ya que el plan no puede existir antes.

Al sistema de archivos que aloja `/usr` se le imputa lo que ocupan los paquetes entrantes menos lo que devuelven los que salen, más un lote atrapado entre sus escrituras y su commit, más los paquetes en uso cuyo espacio solo se libera con los últimos lotes. Al que aloja la caché de descargas se le imputan los RPM que faltan por recuperar. Cuando `/var` es una partición aparte, cada uno tiene su propio veredicto; en un solo sistema de archivos tampoco se suman, porque los RPM de cada lote se borran en cuanto se instala: la caché se vacía al ritmo en que llegan los ficheros nuevos y el pico es el mayor de los dos.

Junto a esa estimación — nunca dentro — hay un margen aconsejado del 10% de la huella instalada, para lo que ningún paquete declara: la initramfs regenerada, las cachés de fuentes e iconos. Incluirlo en la necesidad convertía una medida en un veredicto. Los bloques que un sistema de archivos reserva para root no cuentan como disponibles. Si el margen es escaso o el sistema de archivos francamente demasiado pequeño, obtiene las cifras y las órdenes que liberan espacio allí donde realmente falta, y decide usted.
