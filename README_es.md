...
Antes de cualquier descarga, el plan se mide frente a cada sistema de archivos que debe alojar una parte. Al que aloja `/usr` se le imputa lo que ocupan los paquetes entrantes menos lo que devuelven los que salen, más los paquetes en uso cuyo espacio solo se libera con los últimos lotes. Al que aloja la caché de descargas se le imputan los RPM que faltan por recuperar. Cuando `/var` es una partición aparte, cada uno tiene su propio veredicto — sumarlos rechazaría una actualización cuyos paquetes caben holgadamente en `/var` porque `/usr` va justo. En un solo sistema de archivos tampoco se suman: los RPM de cada lote se borran en cuanto queda instalado, así que la caché se vacía al ritmo en que llegan los ficheros nuevos y el pico es el mayor de los dos, no su suma. Encima se añaden un lote atrapado entre sus escrituras y su commit, y una provisión declarada del 10% de la huella instalada para lo que ningún paquete describe — la salida de los scriptlets, el crecimiento del rpmdb. Esa provisión se muestra en su propia línea en vez de diluirse en el total. Los bloques que un sistema de archivos reserva para root no cuentan como disponibles. Si uno de los dos no cabe, el cambio de repositorios se deshace y se muestran las cifras junto con las órdenes que liberan espacio allí donde realmente falta. Lo que generan los scriptlets — initramfs, cachés de fuentes e iconos — no pertenece a ningún paquete y no se cuenta.
