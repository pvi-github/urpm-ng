...
### Atualização de distribuição (mga N → N+1)

```bash
urpm distupgrade                        # Detecção automática do destino = atual + 1
urpm distupgrade --to 11                # Versão de destino explícita
urpm distupgrade --to cauldron          # Destino Cauldron (rolling)
urpm distupgrade --to cauldron:12       # Cauldron apontando para 12 (durante freeze)

# Opções
--to <version>                # Destino explícito ; auto = atual + 1
--yes / -y / --auto           # Ignora confirmações, remove media antigos
                              # automaticamente no Stage 4
--dry-run                     # Só verificações Stage 0 + atualização de metadados
--export-plan <file>          # Resolver, escrever NEVRAs em <file>, restaurar
                              # a BD (sem descarga, sem instalação) — combinar
                              # com `urpm download --from-file` num peer
                              # vizinho para pré-carregar a cache antes do
                              # distupgrade real
--resume                      # Retomar um distupgrade interrompido
--abort                       # Abandonar um distupgrade em curso ou interrompido

# Limpeza pós-atualização
urpm media remove --distupgraded   # Remove os repositórios mga N substituídos
                                    # pela sua contraparte mga N+1
```

Executa em fases : verificações preliminares (detecção de versão, maturidade do destino, prompt de salto multi-versão) → mudança de repositórios → resolução do plano da versão de destino + prompt de confirmação → descarga → instalação dos componentes críticos do sistema (rpm / python / glibc) primeiro, depois o resto → relatório pós-transação (ficheiros `.rpmnew`, pacotes mga N residuais, media de terceiros órfãos). Reinicie para que os ajustes pós-arranque sejam executados no próximo arranque.

Antes de qualquer descarga, o plano é medido face a cada sistema de ficheiros que tem de alojar uma parte, e os valores são apresentados junto ao pedido de confirmação. Nada é recusado: a estimativa está calibrada sobre uma única migração medida, e uma recusa errada abortaria uma atualização que tinha tudo para correr bem — depois de os repositórios já terem sido trocados, pois o plano não pode existir antes disso.

Ao sistema de ficheiros que aloja `/usr` é imputado o que os pacotes que chegam ocupam menos o que devolvem os que saem, mais um lote apanhado entre as suas escritas e o seu commit, mais os pacotes ainda em uso cujo espaço só volta com os últimos lotes. Ao que aloja a cache de descargas são imputados os RPM que faltam obter. Quando `/var` é uma partição separada, cada um tem o seu próprio veredicto; num único sistema de ficheiros também não se somam, porque os RPM de cada lote são apagados assim que instala: a cache esvazia-se ao ritmo a que os ficheiros novos chegam e o pico é o maior dos dois.

Ao lado dessa estimativa — nunca dentro — está uma margem aconselhada de 10% da pegada instalada, para o que nenhum pacote declara: a initramfs regenerada, as caches de tipos de letra e ícones. Incluí-la na necessidade transformava uma medição num veredicto. Os blocos que um sistema de ficheiros reserva para o root não contam como disponíveis. Se a margem for curta ou o sistema de ficheiros claramente pequeno demais, obtém os valores e os comandos que libertam espaço onde falta de facto, e decide.
