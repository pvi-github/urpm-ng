...
Antes de qualquer descarga, o plano é medido face a cada sistema de ficheiros que tem de alojar uma parte. Ao que aloja `/usr` é imputado o que os pacotes que chegam ocupam menos o que devolvem os que saem, mais os pacotes ainda em uso cujo espaço só volta com os últimos lotes. Ao que aloja a cache de descargas são imputados os RPM que faltam obter. Quando `/var` é uma partição separada, cada um tem o seu próprio veredicto — somá-los recusaria uma atualização cujos pacotes cabem folgadamente em `/var` só porque `/usr` está apertado. Num único sistema de ficheiros também não se somam: os RPM de cada lote são apagados assim que fica instalado, portanto a cache esvazia-se ao ritmo a que os ficheiros novos chegam e o pico é o maior dos dois, não a sua soma. A isso juntam-se um lote apanhado entre as suas escritas e o seu commit, e uma provisão declarada de 10% da pegada instalada para o que nenhum pacote descreve — o que os scriptlets produzem, o crescimento do rpmdb. Essa provisão é mostrada na sua própria linha em vez de se diluir no total. Os blocos que um sistema de ficheiros reserva para o root não contam como disponíveis. Se um deles não couber, a mudança de repositórios é revertida e os valores são apresentados com os comandos que libertam espaço onde falta de facto. O que os scriptlets geram — initramfs, caches de tipos de letra e ícones — não pertence a nenhum pacote e não é contado.
